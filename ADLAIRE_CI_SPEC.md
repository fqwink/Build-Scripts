...
13章の項目は、以下のリスト単位で読む。実装可否はリストごとに判断し、別リストの項目と混同してはならない。

| リスト | 状態 | 実装可否 | 意味 | 次に見る仕様 |
|--------|------|----------|------|--------------|
| 13.2 実装済みリスト | 実装済み | 完了済み | ソースコード実装と検証が完了した項目。 | 実装ファイル、検証結果、`DOCUMENT_INDEX.md` |
| 13.3 改訂予定リスト | 改訂予定 | 実装不可 | 将来対応から格上げ済みだが、詳細仕様作成中の項目。 | `ADLAIRE_CI_DETAIL_SPEC.md` の改訂対象節 |
| 13.4 仕様化済み・未実装リスト | 仕様化済み・未実装 | 実装可 | 正本仕様と詳細仕様があり、実装対象として扱える項目。 | `ADLAIRE_CI_DETAIL_SPEC.md` §0h・§0i |
| 13.5 将来対応リスト | 将来対応 | 実装不可 | 方向性または候補として記録した項目。 | 本節 13.7 の昇格手順 |
| 13.6 MCP サーバー将来計画リスト | 将来計画 | 実装不可 | `mcp_server.go` 関連の将来構想。 | 本節 13.7 の昇格手順 |

将来対応、将来計画、改訂予定の項目は、実装着手可能な仕様ではない。実装対象にする場合は、先に 13.7 の手順で仕様化済み・未実装へ昇格させる。
...
格上げ済み項目の一覧。仕様化完了後は `ADLAIRE_CI_SPEC.md` または `ADLAIRE_CI_DETAIL_SPEC.md` の各仕様節へ反映し、仕様化済み・未実装リストへ移動する。
...
`将来対応` は、方向性または候補の記録であり、実装対象ではない。実装対象へ昇格する場合は、先に 13.3 改訂予定リストへ移動し、`ADLAIRE_CI_DETAIL_SPEC.md` に入力、出力、状態、処理順序、異常系、検証条件を追加してから `仕様化済み` へ変更する。

本リストの項目は、実装条件、API、状態ファイル、UI、検証条件を確定済みとして扱わない。実装したい場合は 13.7 の昇格手順を必ず実施する。
...
本リストは、MCP 関連の構想を他の将来対応と分けて管理するためのリストである。ここに記載された API、状態ファイル、ツール名、認証方式は、詳細仕様化されるまで実装不可とする。
...
将来対応リストまたは MCP サーバー将来計画リストの項目を実装対象にする場合は、以下の順で進める。

1. 対象項目を 13.3 改訂予定リストへ移動し、元区分、格上げ日、担当領域、優先度、ステータスを記録する。
...
6. 仕様凍結条件を満たした後、13.4 仕様化済み・未実装リストへ移動する。
7. 実装と検証が完了した後、13.2 実装済みリストへ移動する。
