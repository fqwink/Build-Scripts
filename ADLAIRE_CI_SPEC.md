...
| 完了条件 | `ADLAIRE_CI_DETAIL_SPEC.md` §0e および §26.7 の受け入れ条件で、実装完了を判定できる。 |
...
- `ADLAIRE_CI_DETAIL_SPEC.md` §0e の完全実装検証マトリクスを満たしている。
...
| 実装コード変更 | 必要に応じて更新 | `N` を 1 以上更新 | 実装 PR で検証結果を記録する。 |
| 安定版リリース | `V.N` を必要に応じて更新 | `X` と `N` を 1 以上更新 | GitHub Release と tag を作成する。 |
