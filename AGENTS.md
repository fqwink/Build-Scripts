...
実装中・検証未完了コンポーネントである `components/api.go` は、`ADLAIRE_CI_SPEC.md`、`ADLAIRE_CI_DETAIL_SPEC.md`、該当する owner component 別の `ADLAIRE_CI_DETAIL_*_SPEC.md` に基づいて更新する。

仕様化済み・未実装コンポーネントである `admin/adlaire-ci-sdk.js`、`admin/index.html` は、`ADLAIRE_CI_SPEC.md`、`ADLAIRE_CI_DETAIL_SPEC.md`、該当する owner component 別の `ADLAIRE_CI_DETAIL_*_SPEC.md` に基づいて更新する。
...
文書整理では、`README.md`、`DOCUMENT_INDEX.md`、`ADLAIRE_CI_SPEC.md`、`ADLAIRE_CI_DETAIL_SPEC.md`、owner component 別の `ADLAIRE_CI_DETAIL_*_SPEC.md` の読み順と正本範囲を維持する。

文書整理だけを目的とする作業では、機能仕様、実装状態、実装可否、ロードマップ状態を変更してはならない。状態変更が必要な場合は、変更対象、変更理由、影響範囲を別途提示し、承認を得る。

`README.md` は初見向けの入口、`DOCUMENT_INDEX.md` は索引、`ADLAIRE_CI_SPEC.md` は方針・ポリシー・状態の正本、`ADLAIRE_CI_DETAIL_SPEC.md` は詳細仕様入口、owner component 別の `ADLAIRE_CI_DETAIL_*_SPEC.md` は詳細仕様本文として扱う。
...
実装済みとして扱うには、コード変更、仕様との差分確認、必要な構文確認、実行確認または生成物確認、`DOCUMENT_INDEX.md`・実装状態表・関連仕様の更新要否確認を完了していなければならない。
