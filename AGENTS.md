...
本リポジトリの仕様判断は、方針、ポリシー、実装状態、ロードマップは `docs/ADLAIRE_CI_SPEC.md`、詳細仕様の入口、索引、共通固定値、実装前確認項目、検証マトリクス、Phase、詳細節対応表、リポジトリ内ソース配置、責務 component 別詳細仕様ファイル管理仕様、横断補足契約は `docs/ADLAIRE_CI_DETAIL_SPEC.md`、各 owner component の詳細本文は `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` を正本として行う。
...
`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、または該当する owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` と実装ファイルが仕様上矛盾する場合は、仕様と実装の不整合として扱う。仕様を変更する場合は、先に該当する仕様書を改訂し、その内容に基づいて実装を更新する。
...
`docs/ADLAIRE_CI_SPEC.md` は、Adlaire CI の方針、ポリシー、実装状態、正本関係を定めるマスター仕様書正本である。

`docs/ADLAIRE_CI_DETAIL_SPEC.md` は、`docs/ADLAIRE_CI_SPEC.md` の Part 3 詳細仕様の入口、索引、共通固定値、実装前確認項目、検証マトリクス、Phase、詳細節対応表、リポジトリ内ソース配置、責務 component 別詳細仕様ファイル管理仕様、横断補足契約を持つ正本である。

owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` は、各 component の詳細仕様本文に関する正本である。
...
実装済みコンポーネントである `components/api.go`、`admin/adlaire-ci-sdk.js`、`admin/index.html` は、`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、該当する owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` に基づいて更新する。
...
`COMMON`、`CORE`、`BASE`、`SHARED`、`FOUNDATION`、その他の横断共通基盤ファイルは、詳細仕様ファイルとして作成してはならない。横断する固定値、読み順、対応表、横断補足契約は `docs/ADLAIRE_CI_DETAIL_SPEC.md` の入口・索引・共通固定値・管理仕様として扱い、component として扱わない。

`docs/DESIGN.md` は、出力 HTML のデザイン仕様を整理する補助文書である。`docs/ADLAIRE_CI_SPEC.md` と矛盾する場合は、`docs/ADLAIRE_CI_SPEC.md` を優先する。

仕様改訂では、既存仕様、`docs/DOCUMENT_INDEX.md`、実装ファイルとの整合性を確認する。

文書整理では、`README.md`、`docs/DOCUMENT_INDEX.md`、`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` の読み順と正本範囲を維持する。

文書整理だけを目的とする作業では、機能仕様、実装状態、実装可否、ロードマップ状態を変更してはならない。状態変更が必要な場合は、変更対象、変更理由、影響範囲を別途提示し、承認を得る。

`README.md` は初見向けの入口、`docs/DOCUMENT_INDEX.md` は索引、`docs/ADLAIRE_CI_SPEC.md` は方針・ポリシー・状態の正本、`docs/ADLAIRE_CI_DETAIL_SPEC.md` は詳細仕様入口、owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` は詳細仕様本文として扱う。

`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` に記載された一部コンポーネントや機能は、仕様化済みであっても未実装の場合がある。リポジトリ内に実装ファイルまたは実装コードが存在しない内容を、実装済み機能として扱ってはならない。

`docs/ADLAIRE_CI_DETAIL_SPEC.md` または owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` を改訂する場合は、方針、ポリシー、実装状態、実装可否、ロードマップ状態、PR 分割判断を記載してはならない。これらは `docs/ADLAIRE_CI_SPEC.md` を正とする。

`docs/ADLAIRE_CI_DETAIL_SPEC.md` と owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` には、実装者が実装時に必要とする対象コンポーネント、入出力、設定値、データ構造、処理順序、異常系、状態管理、セキュリティ制約、検証条件だけを記載する。

未確定の内容を実装可能な詳細仕様として扱ってはならない。実装判断に必要な具体値、条件、処理が未確定の場合は、`docs/ADLAIRE_CI_SPEC.md` の状態分類を確認し、`docs/ADLAIRE_CI_DETAIL_SPEC.md` または owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` へ推測で具体値を記載してはならない。

仕様項目の成熟度と実装可否は、`docs/ADLAIRE_CI_SPEC.md` Part 1 §4b および Part 2 §0a の仕様成熟度方針・仕様成熟度ポリシーに従って判定する。

作業開始時には、対象機能・対象コンポーネントについて `docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、該当する owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` の該当節と実ファイルの存在を確認する。
...
未実装コンポーネントを追加する場合は、`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、該当する owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md`、`docs/DOCUMENT_INDEX.md`、本ファイルを必要に応じて整合させる。
...
自動化に関わる設定には、少なくとも `delete_branch_on_merge=true` を含める。その他の自動化設定が `docs/ADLAIRE_CI_SPEC.md` または本ルールブックで標準化された場合も同様に扱う。
...
- 仕様変更では、`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、該当する owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md`、`docs/DOCUMENT_INDEX.md`、`docs/DESIGN.md`、実装ファイルの整合を確認する。
...
例外採用は、`docs/ADLAIRE_CI_SPEC.md` または承認済み変更範囲に明記された場合のみ有効とする。
...
`docs/ADLAIRE_CI_SPEC.md`、`docs/ADLAIRE_CI_DETAIL_SPEC.md`、または owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` を改訂した場合は、`docs/DESIGN.md`、`docs/DOCUMENT_INDEX.md`、実装ファイルへの影響を確認する。

`docs/DESIGN.md` を改訂した場合は、`components/builder.go` 内の HTML / CSS / JavaScript / theme component テンプレートとの整合性を確認する。

仕様化済み項目を実装した場合は、`docs/ADLAIRE_CI_SPEC.md` 内の状態表現、`docs/DOCUMENT_INDEX.md` の Specified Components、`docs/ADLAIRE_CI_DETAIL_SPEC.md` の対応表、owner component 別の `docs/ADLAIRE_CI_DETAIL_*_SPEC.md` の検証条件、実装ファイルの存在を整合させる。
