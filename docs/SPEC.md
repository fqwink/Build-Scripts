...
Adlaire CI の仕様体系は、責務ベース明示的原則を仕様全般の上位方針として採用する。

責務ベース明示的原則とは、方針、ポリシー、実装状態、Phase、将来計画、詳細仕様本文、fixture、expected、fake、PR 証跡、文書索引を、それぞれ異なる責務として明示的に分離し、同一判断対象を複数文書で正本化しない原則である。

分かりやすさは、同じ説明を複数文書へ重複記載することで担保してはならない。分かりやすさは、責務、正本範囲、参照先、参照節を明示することで担保する。

他責務に属する内容は、本文として再定義せず、正本への参照で接続する。参照先は、可能な限りファイル名と節番号で示す。
...
他責務の内容を説明する必要がある場合は、本文を再定義せず、正本ファイルと節番号を参照する。参照だけでは実装者が判断できない場合は、参照先の正本を先に改訂し、参照元へ重複本文を追加して補完してはならない。
