...
3. Part 1 §4.1〜§4.3、§4a〜§4g で、ゼロ依存、責務ベース明示的原則、リポジトリ構成、詳細仕様粒度、成熟度、着手ゲート、完了判定、Go 正本方針を確認する。
...
`docs/DETAIL_INDEX.md` と owner component 別の `docs/details/*.md` は、Part 1 §4.2a、§4a と Part 2 §0 に定める範囲に従い、実装者が迷わない入出力、状態、処理順序、異常系、検証条件だけを維持する。
