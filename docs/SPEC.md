...
本ファイルは、関数単位の処理、HTTP response schema、状態ファイル schema、SDK method、UI DOM、fixture assertion、具体的な実行手順を定義しない。これらの本文は、Part 1 §4.2a と Part 3 に定める責務正本へ分離する。
...
| Roadmap | 状態・計画 | **いつ・どれを** | 状態責務。実装状態、Phase、機能インベントリ、将来計画、昇格手順は `docs/ROADMAP.md` を正本とする |
| Part 3 | 仕様 | **どのように** | 詳細仕様責務。入口は `docs/DETAIL_INDEX.md`、owner component 本文は `docs/details/*.md` を正本とする |
...
3. Part 1 §4.1〜§4.10 で、ゼロ依存、責務ベース明示的原則、リポジトリ構成、詳細仕様粒度、成熟度、着手ゲート、完了判定、Go 正本方針を確認する。
...
- 追加時に `docs/DOCUMENT_INDEX.md`、`docs/ROADMAP.md` の実装状態、Part 2 §4 の内製スクリプト一覧、`docs/DETAIL_INDEX.md` の詳細仕様と受け入れ条件を整合する。
...
分かりやすさは、同じ説明を複数文書へ重複記載することで担保してはならない。分かりやすさは、責務、正本範囲、参照先、参照節を明示することで担保する。判断に迷う記載、二重に読める記載、例外に見える記載、参照先を持たない責務外説明は禁止する。
...
実装順序、実装計画、実装 PR、完了判定は、本ファイル §4.6 の Phase 実装単位方針と Part 2 §0f の Phase 実装単位ポリシーに従う。
...
詳細仕様、状態管理、fixture、索引、入口文書を記載または整理する場合は、Part 1 §4.2a の責務ベース明示的原則に従わなければならない。
...
# Part 3 — 仕様
> 実装の具体的詳細は、本ファイルへ重複記載せず、責務を持つ詳細仕様文書を正本とする。

Part 3 の入口、読み順、共通固定値、実装前確認項目、検証マトリクス、詳細節対応表、リポジトリ内ソース配置、責務 component 別詳細仕様ファイル管理仕様は `docs/DETAIL_INDEX.md` を正本とする。

各 owner component の入出力、状態、処理順序、異常系、検証条件の本文は `docs/details/*.md` を正本とする。

状態責務は `docs/ROADMAP.md` を正本とする。

`docs/DETAIL_INDEX.md` と owner component 別の `docs/details/*.md` は、Part 1 §4.2a、§4.4 と Part 2 §0 に定める範囲に従い、方針、ポリシー、実装状態、実装可否、ロードマップ状態、PR 分割判断を本文として記載してはならない。
