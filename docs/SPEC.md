...
`docs/SPEC.md` の `技術方針` 表と `リポジトリ構成方針` のディレクトリ構成 tree は、方針責務の本文である。これらを実装詳細、状態一覧、索引本文とみなして削除、移動、参照化してはならない。
...
本節のディレクトリ構成 tree は、Adlaire CI の構成方針を明示する正本であり、削除または他文書への移動を禁止する。`docs/DOCUMENT_INDEX.md` は所在索引として本節の方針に従う。
