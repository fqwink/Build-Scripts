...
`docs/SPEC.md` の `技術方針` 表と `ディレクトリ構成` tree は、Adlaire CI の明示的方針であり、削除してはならない。これらは実装詳細、状態一覧、索引本文として扱わない。
...
3. `docs/SPEC.md` 方針責務 §4.1〜§4.10 で、ゼロ依存、責務ベース明示的原則、リポジトリ構成、詳細仕様粒度、成熟度、着手ゲート、完了判定、Go 正本方針を確認する。
...
本表は Adlaire CI の技術選定に関する明示的方針であり、削除または他文書への移動を禁止する。詳細仕様は本表の方針に従い、具体的な入出力、処理順序、例外条件、検証条件を owner component 詳細本文責務へ記載する。
...
`docs/SPEC.md` の `技術方針` 表と `ディレクトリ構成` tree は、方針責務の本文である。これらを実装詳細、状態一覧、索引本文とみなして削除、移動、参照化してはならない。
...
本節のディレクトリ構成 tree は、Adlaire CI のディレクトリ構成を明示する正本であり、削除または他文書への移動を禁止する。`docs/DOCUMENT_INDEX.md` は所在索引として本節に従う。
