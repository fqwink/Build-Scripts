...
§28.1〜§28.25 を実装する場合は、個別行の受け入れ条件に加えて、`docs/details/builder.md` §28 の共通固定契約、CLI / 設定 / REPORT / 出力識別子固定契約、実装パイプライン固定契約、設定解決・終了コード固定契約、REPORT 値型固定契約、CSS / JS 出力固定契約、Markdown token / HTML node 変換固定契約、warning / error code 固定契約、既存出力互換・先取り実装禁止固定契約、個別固定補足契約、および `docs/details/fixture.md` §28-F を必ず読む。
