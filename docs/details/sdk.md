...
[`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 は、sdk owner の JavaScript SDK 詳細本文責務である。
...
| JSON 受信 | `Content-Type` が JSON の場合のみ `response.json()` を呼ぶ。[`docs/details/api.md`](api.md) 詳細本文責務 §22.0e で JSON response を定義した endpoint の成功時に空 body を受信した場合は protocol error として `AdlaireCIError(status=0, message="Empty JSON response")` を投げる。 |
...
| endpoint 対応 | SDK method は [`docs/details/api.md`](api.md) 詳細本文責務 §22.0e の SDK 列に存在する endpoint だけを呼び出す。[`docs/details/api.md`](api.md) 詳細本文責務 §22.0e にない endpoint を SDK 独自判断で追加してはならない。 |
| body なし endpoint | [`docs/details/api.md`](api.md) 詳細本文責務 §22.0e の `Request` が `none` の場合、SDK は `fetch` に `body` を設定しない。`{}` も送信しない。 |
...
HTTP status と SDK error の対応は [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 の固定表に固定する。
...
| 項目 | 仕様 |
|------|------|
| public method 定義順 | class 内の public method は [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 の一覧順に定義する。追加 public method を末尾に置くことは禁止し、先に [`docs/details/api.md`](api.md) 詳細本文責務 §22.0e と本一覧を更新する。 |
...
| query parameter | query key は [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 SDK 引数変換契約の表記順で生成する。任意 query が未指定の場合、`?` 自体を付けない。 |
| body parameter | body object の key 順は [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 SDK 引数変換契約の送信値順とする。未知 key を SDK が追加しない。 |
...
| 検証項目 | 合格条件 |
|----------|----------|
| endpoint coverage | [`docs/details/api.md`](api.md) 詳細本文責務 §22.0e の SDK 列で `none` 以外の method がすべて `AdlaireCI.prototype` に存在する。 |
| extra method | [`docs/details/api.md`](api.md) 詳細本文責務 §22.0e の SDK 列に存在しない public method がない。 |
| request shape | 各 method が [`docs/details/api.md`](api.md) 詳細本文責務 §22.0e の Request と [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 SDK 引数変換契約どおりの path / query / body を生成する。 |
...
| ゲート | 合格条件 |
|--------|----------|
| method coverage | [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §27.21〜§27.47 の api / sdk / ui 連動参照表で SDK method が記載された項目について、該当 public method が存在する。 |
| request exactness | fake fetch fixture で method、path、query key 順、body key 順、body なし endpoint が [`docs/details/sdk.md`](sdk.md) 詳細本文責務 §23 と一致する。 |
