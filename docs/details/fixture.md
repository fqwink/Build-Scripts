...
| <a id="align-29"></a>`ALIGN-29` | output manifest / filesystem identity | [`components/runner.go`](../../components/runner.go) の `outputManifestSHA()` と [`components/api.go`](../../components/api.go) の `inspectOutput()` は `os.Stat` / `filepath.WalkDir` / 通常 open で出力を走査し、root symlink、symlink entry、hardlink、device、socket、FIFO、不正相対 path、読取中の identity / size / mtime 変更を拒否しない。API は directory でない root も出力として受理し、runner と API が同一 manifest を算出する共通 adapter を使用しない。 | [`docs/DETAIL_INDEX.md` 詳細仕様入口責務 §0d 共通固定値](../DETAIL_INDEX.md#0d-共通固定値) の出力成果物 manifest SHA-256 契約に従い、no-follow open、file type / link count / path / identity / size / mtime 検証、空 directory digest、runner / API 同一結果、走査中変更の失敗を固定する cross fixture。 |
